            ...
            <Reveal key={p.name} delay={i * 0.12}>
              <article className="grain-card group relative h-full overflow-hidden rounded-3xl p-8 md:p-10">
                <div
                  ...
                  style={{ background: p.accent === "gold" ? "#75C4E5" : "#373B99" }}
                />
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 font-serif text-3xl tracking-tight md:text-4xl">{p.name}</h3>
                    <div className="mt-1 text-paper/60">{p.role}</div>
                  </div>
                  <div
                    className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full font-serif text-3xl font-black text-ink-950"
                    style={{ background: p.accent === "gold" ? "#75C4E5" : "#373B99" }}
                  >
                ...
                <div className="mt-8">
                  <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-paper/40">
                    {p.pubs[0]}
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {p.pubs.slice(1).map((pub) => (
                      <span
                        key={pub}
                        className="rounded-full border border-paper/15 px-3 py-1 font-serif text-sm text-paper/80"
                      >
                        {pub}
                      </span>
                    ))}
